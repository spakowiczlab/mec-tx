...
#' @examples
#' \dontrun{
#' # Basic CAlevel KM for full cohort
#' p <- km_panel_from_df(
#'   df    = Cluster_surv,
#'   title = "Overall Survival by CAlevel — LUSC"
#' )
#' pdf(file.path(out_dir, "km_calevel_lusc.pdf"), width = 10, height = 8)
#' print(p)
#' dev.off()
#'
#' # Clipped to 5 years, no risk table
#' p2 <- km_panel_from_df(
#'   df            = Cluster_surv,
#'   horizon_years = 5,
#'   risk_table    = FALSE,
#'   title         = "OS by CAlevel — 5-year window"
#' )
#'
...
#' @importFrom stats pchisq as.formula pretty
#' @export
